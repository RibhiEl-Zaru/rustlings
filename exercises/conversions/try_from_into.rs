...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        if tuple.0 < 0 || tuple.0 > 255 ||  tuple.1 < 0 || tuple.1 > 255 ||  tuple.2 < 0 || tuple.2 > 255 {
            Err("all color values must be in the 0..=255 range".to_string())
        } else {
...
impl TryFrom<[i16; 3]> for Color {
    ...
    fn try_from(arr: [i16; 3]) -> Result<Self, Self::Error> { 
        for num in arr.into_iter() {
            if num < &0 || num > &255 {
                return Err("all color values must be in the 0..=255 range".to_string());
            }
        }
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> { 
        if slice.len() != 3 { return Err("The passed in slice is not 3 numbers. Pass only 3 numbers into it.".to_string())}
